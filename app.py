...
def train_model(intent_data: List[Dict]):
    ...
    # Collect all unique intents (supports list or single string)
    all_intents = set()
    for item in intent_data:
        if isinstance(item["intent"], list):
            all_intents.update(item["intent"])
        else:
...
def predict(model, tokenizer, idx_to_intent, text):
    ...
    with torch.no_grad():
        ...
        for i, p in enumerate(probs):
            ...
            results.append((idx_to_intent[i], float(p)))
...
if __name__ == "__main__":
    ...
    while True:
        text = input("\nYou: ")
        results = predict(model, tokenizer, idx_to_intent, text)
